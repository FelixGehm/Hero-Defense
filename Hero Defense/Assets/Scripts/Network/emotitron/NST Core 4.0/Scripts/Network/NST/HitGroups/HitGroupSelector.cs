...
public struct HitGroupSelector
{
	...
	{
		List<string> tags = NSTHitGroupsSettings.single.hitGroupTags;
		...
		if (hitGroupTagId < tags.Count)
		{
			...
			for (int i = 0; i < tags.Count; i++)
			{
				...
				{
					hitGroupTagId = i;
					DebugX.LogWarning(!DebugX.logWarnings ? "" : (_parent.name + " references hit group named '" + hitGroupTag + "', but that tag is no longer at the same index. "+
			...
			DebugX.LogWarning(!DebugX.logWarnings ? "" : (_parent.name + " references hit group tag named '" + hitGroupTag + "', but that tag does not exist anymore. " +

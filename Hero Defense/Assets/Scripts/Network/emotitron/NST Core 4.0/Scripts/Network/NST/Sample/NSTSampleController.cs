...
namespace emotitron.Network.NST.Sample
{
	...
	/// </summary>
	public class NSTSampleController : NSTComponent
	{
		...
		void Start()
		{
			...
			// Find position and rotation elements that are defined for this child
		...
		private void ApplyPosition(Vector3 moves)
		{
			...
			else
			{
				...
				Vector3 clamped = 
					...
					(restrictToNstRange && isRootGO) ? WorldVectorCompression.ClampAxes(unclamped) :
					unclamped;

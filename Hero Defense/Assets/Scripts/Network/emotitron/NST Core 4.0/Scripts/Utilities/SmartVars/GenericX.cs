...
namespace emotitron.Utilities.SmartVars
{
	...
	/// <summary>
	/// A generic position/rotation struct that freely converts between quaternions, Eulers and partial Eulers. XType value indicates which axes
	/// are included, so operations can be done without affecting the unindicated axes. For example if you you try to rotate between two GenericX values
	/// both with an XType of XY ... the Z axis will not be affected.
	/// </summary>

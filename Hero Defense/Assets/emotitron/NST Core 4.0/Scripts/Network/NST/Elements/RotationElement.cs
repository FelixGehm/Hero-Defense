...
namespace emotitron.Network.NST
{
	...
	public bool useLocal = true;
		...
		public override bool Write(ref UdpBitStream bitstream, Frame frame)
		{
			...
			frame.elements[index].compTrans = Compress();
			frame.elements[index].transform = Localized;
			CompressedElement newComp = frame.elements[index].compTrans;
			...
			if (rotationType == RotationType.Quaternion)
			{
				...
				lastSentTransform = frame.elements[index].transform;
		...
		public uint CompressFloat(float f, int axisId)
		{
			...
			if (adjusted > axes[axisId].range && adjusted < axes[axisId].wrap)
				return (uint)maxValue[axes[axisId].bits];
		...
		public override bool Read(ref UdpBitStream bitstream, Frame frame, Frame currentFrame)
		{
			...
			if (!hasChanged)
			{
				...
				if (!isCurrentFrame)
				{
					frame.elements[index].transform = GenericX.NULL;
					frame.elements[index].compTrans = CompressedElement.zero;
				}
				return false;
			}
			...
			if (rotationType == RotationType.Quaternion)
			{
				frame.elements[index].compTrans = bitstream.ReadULong(totalBitsForQuat);
				frame.elements[index].transform = frame.elements[index].compTrans.quat.DecompressBitBufferToQuat(totalBitsForQuat);
			}
			else
			{
				for (int axisId = 0; axisId < 3; axisId++)
				...
				frame.elements[index].transform = new GenericX(vals[0], vals[1], vals[2], (XType)includedAxes);
				frame.elements[index].compTrans = new CompressedElement(cvals[0], cvals[1], cvals[2]);
			}

			if (applyToGhost)
				frame.elements[index].transform.ApplyRotation(rewindGO.transform, useLocal);
		...
		public override void UpdateInterpolation(float t)
		{
			...
			Apply(Quaternion.Slerp(snapshot.elements[index].transform, target.elements[index].transform, t));
		}

		/// <summary>
		/// Apply a rotation to a gameobject, respecting this elements useLocal and axis restrictions (specified by the GenericX type)
		/// </summary>
		/// <param name="rot"></param>
		public override void Apply(GenericX rot)
		{
	...
	public class RotationElementDrawer : TransformElementDrawer
	{
		...
		public bool DrawAxis(int axisId, SerializedProperty includedAxes)
		{
			...
			GUI.Label(new Rect(left, currentLine - 1, realwidth - 16, LINEHEIGHT), "Bits used: " + bitcount, "MiniLabelRight");

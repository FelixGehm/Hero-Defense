...
namespace emotitron.Utilities.GenericCast
{
	...
	public static class GenericCastExt
	{
		...
		{
			int hitcount;
			Vector3 srcPos = (useOffset) ? srcT.TransformPoint(offset1) : srcT.position;
			...
			switch (casttype)
			{
				...
				case CastType.CapsuleCast:
					hitcount = Physics.CapsuleCastNonAlloc(srcPos, srcT.TransformPoint(offset2), radius, srcT.forward, rayhits, distance, mask);
					break;
				...
				case CastType.OverlapCapsule:
					hitcount = Physics.OverlapCapsuleNonAlloc(srcPos, srcT.TransformPoint(offset2), radius, hits, mask);
					break;
